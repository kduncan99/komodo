...
public class SSFunction extends BuiltInFunction {
    ...
    public Value evaluate(
        ...
        if ((arguments.length == 3) && (arguments[2].getType() != ValueType.Integer)) {
            assembler.appendDiagnostic(getValueDiagnostic(3));
            throw new ExpressionException();
        }
        ...
        if (iarg1._value.get().compareTo(BigInteger.ONE) < 0) {
            assembler.appendDiagnostic(new ValueDiagnostic(getLocale(), "Index argument must be > 0"));
            throw new ExpressionException();
        }

        if (iarg1._value.get().compareTo(BigInteger.valueOf(0_777777)) < 0) {
            assembler.appendDiagnostic(new ValueDiagnostic(getLocale(), "Index argument cannot be greater than 0777777"));
            throw new ExpressionException();
        }

        if (iarg2 != null) {
            if (iarg2._value.get().compareTo(BigInteger.ONE) < 0) {
                assembler.appendDiagnostic(new ValueDiagnostic(getLocale(), "Count argument must be > 0"));
                throw new ExpressionException();
            }

            if (iarg2._value.get().compareTo(BigInteger.valueOf(0_777777)) < 0) {
                assembler.appendDiagnostic(new ValueDiagnostic(getLocale(), "Count argument cannot be greater than 0777777"));
                throw new ExpressionException();
            }
        }
