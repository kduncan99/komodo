...
import com.bearsnake.komodo.kexec.consoles.*;
...
public class Exec extends ExecRun {
    ...
    public String dump(boolean verbose) {
        ...
        try {
            ...
            LOGGER.error("Failed to create panic dump file", ex);
            if (isRunning()) {
    ...
    public String sendExecReadReplyMessage(
        ...
        if (rrmsg.isCanceled()) {
            LOGGER.info("Console message is canceled");
            throw new ExecStoppedException();
        }
    ...
    private String sendExecRestrictedReadReplyMessage(
        ...
        if (candidates.length == 0) {
            LOGGER.fatal("sendExecRestrictedReadReplyMessage:Empty candidates list");
