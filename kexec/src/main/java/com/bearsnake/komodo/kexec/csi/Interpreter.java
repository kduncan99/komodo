...
public class Interpreter {
    ...
    private static final Map<String, CSIHandler> HANDLERS = new HashMap<>();
    static {
    ...
    private static void addHandler(final CSIHandler handler) {
    ...
    public void handleControlStatement(
        ...
        LOGGER.traceEntry(String.format("HandleControlStatement [%s]", run.getActualRunId()));
        var hp = new HandlerPacket(run, source, statement);
        var handler = HANDLERS.get(statement._mnemonic);
        if (handler == null) {
            LOGGER.warn("[{}] invalid control statement:{}", run.getActualRunId(), statement._originalStatement);
            ...
            LOGGER.warn("[{}] {} not allowed for ER CSI$", run.getActualRunId(), statement._mnemonic);
            ...
            LOGGER.warn("[{}] {} not allowed for ER CSF$", run.getActualRunId(), statement._mnemonic);
            run.postContingency(012, 04, 042);
            statement._facStatusResult.postMessage(FacStatusCode.IllegalControlStatement);
            hp._statement._facStatusResult.mergeStatusBits(0_400000_000000L);
        } else if ((run instanceof TIPRun) && !handler.allowTIP()) {
            LOGGER.warn("[{}] {} not allowed in TIP program", run.getActualRunId(), statement._mnemonic);
            run.postContingency(012, 04, 042);
            statement._facStatusResult.postMessage(FacStatusCode.IllegalControlStatement);
            hp._statement._facStatusResult.mergeStatusBits(0_400000_000000L);
        } else {
        ...
        LOGGER.trace(String.format("HandleControlStatement [%s] returning code %012o",
                                   run.getActualRunId(),
    ...
    public static ParsedStatement parseControlStatement(
        ...
        LOGGER.traceEntry("[{}] parseControlStatement {}", run.getActualRunId(), statement);
        ...
        if (p.atEnd() || !p.parseChar('@')) {
            LOGGER.warn("[{}] statement does not begin with '@'", run.getActualRunId());
            run.postContingency(012, 04, 040);
        ...
        try {
        ...
        } catch (SyntaxErrorParserException ex) {
            postSyntaxError(ps._facStatusResult);
            return ps;
            ...
            if (!p.atEnd()) {
                ...
                LOGGER.trace("[{}] empty statement", run.getActualRunId());
            }
            return ps;
        }
        ...
        if (isLabel) {
            ...
            try {
                ...
                if (!p.atEnd()) {
                    ...
                    LOGGER.trace("[{}] empty labeled statement", run.getActualRunId());
                }
                return ps;
            }
        }
        ...
        if (ps._mnemonic.equals("LOG") || ps._mnemonic.equals("MSG")) {
            ...
            LOGGER.trace(String.format("[%s] parse control statement exit -> %s", run.getActualRunId(), ps));
            return ps;
        }
    ...
    private static void postSyntaxError(final FacStatusResult fsr) {
        ...
        LOGGER.trace(String.format("parseControlStatement stat=%012o", fsr.getStatusWord()));
    }
}
