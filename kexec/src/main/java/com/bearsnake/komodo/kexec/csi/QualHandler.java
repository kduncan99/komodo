...
class QualHandler extends CSIHandler {

    private static final Logger LOGGER = LogManager.getLogger(QualHandler.class);
    ...
    public void handle(final HandlerPacket hp) {
        ...
        if ((qualifier != null) && !Parser.isValidQualifier(qualifier)) {
            LOGGER.info("[{}] Invalid qualifier:{}", hp._run.getActualRunId(), hp._statement._originalStatement);
            postSyntaxError(hp);
            return;
        }
        ...
        if (hp._optionWord == 0) {
            ...
            if (qualifier == null) {
                LOGGER.info("[{}] Missing qualifier:{}", hp._run.getActualRunId(), hp._statement._originalStatement);
                if (hp._sourceIsExecRequest) {
                    hp._run.postContingency(012, 04, 040);
                }
            ...
            if (qualifier != null) {
                LOGGER.info("[{}] Qualifier should not be specified:{}", hp._run.getActualRunId(), hp._statement._originalStatement);
                if (hp._sourceIsExecRequest) {
                    hp._run.postContingency(012, 04, 040);
                }
