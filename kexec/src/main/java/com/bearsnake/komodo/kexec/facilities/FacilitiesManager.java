...
import com.bearsnake.komodo.hardwarelib.channels.*;
import com.bearsnake.komodo.hardwarelib.devices.*;
...
import com.bearsnake.komodo.kexec.exceptions.*;
import com.bearsnake.komodo.kexec.exec.ERIO$Status;
import com.bearsnake.komodo.kexec.exec.Exec;
import com.bearsnake.komodo.kexec.exec.StopCode;
import com.bearsnake.komodo.kexec.facilities.facItems.*;
import com.bearsnake.komodo.kexec.mfd.*;
...
public class FacilitiesManager implements Manager {
    ...
    private static final Logger LOGGER = LogManager.getLogger(FacilitiesManager.class);
    ...
    public synchronized boolean assignCatalogedDiskFileToExec(
        ...
        LOGGER.traceEntry("assignCatalogedDiskFileToExec({}, {}, {})", fileSpecification, exclusiveUse, fsResult);
        ...
        LOGGER.traceExit("assignCatalogedDiskFileToExec() -> {}", result);
        return result;
    }
    ...
    public synchronized boolean assignCatalogedDiskFileToRun(
        ...
        LOGGER.traceEntry("assignCatalogedDiskFileToRun({}, {}, {})", run, request, fsResult);
        var exec = Exec.getInstance();
        ...
        try {
            ...
            fsResult.log(LOGGER, Level.TRACE);
            return false;
        }
        ...
        if (request._fileSpecification.hasFileCycleSpecification()
            ...
            if (facItem != null) {
                ...
                try {
                    ...
                    LOGGER.fatal("MFD cannot find a file cycle which must exist {}*{}({})", qualifier, filename, absCycle);
        ...
        if (wasAlreadyAssigned) {
            ...
            try {
                ...
                LOGGER.fatal("MFD cannot find a file cycle which must exist {}*{}({})",
                             fsInfo.getQualifier(),
                             fsInfo.getFilename(),
        ...
        LOGGER.trace("assignCatalogedFileToRun {} result:{}", run.getActualRunId(), fsResult.toString());
        fsResult.log(LOGGER, Level.TRACE);
        return true;
    }
    ...
    public synchronized boolean assignDiskUnitToExec(
        ...
        LOGGER.traceEntry("assignDiskUnitToExec {} {} {}", fileSpecification, nodeIdentifier, packName);
    ...
    public synchronized boolean assignDiskUnitToRun(
        ...
        LOGGER.traceEntry("assignDiskUnitToRun {} {} {} {} {} {} {}",
                          run,
                          fileSpecification,
                          nodeIdentifier,
                          packName,
        ...
        if (nodeInfo == null) {
            LOGGER.fatal(String.format("assignDiskUnitToRun() Cannot find node %012o", nodeIdentifier));
        ...
        if ((node.getNodeCategory() != NodeCategory.Device) || ((Device) node).getDeviceType() != DeviceType.DiskDevice) {
            LOGGER.fatal(String.format("assignDiskUnitToRun() Cannot assign to non-disk device %012o", nodeIdentifier));
            Exec.getInstance().stop(StopCode.FacilitiesComplex);
            throw new ExecStoppedException();
        }
        ...
        if (!facItem.isAssigned()) {
            LOGGER.trace("assignDiskUnitToRun promptLoadPack returns false");
            // z-option bail-out
    ...
    public synchronized boolean assignFixedDiskUnitToExec(
        ...
        LOGGER.traceEntry("assignFixedDiskUnitToExec {} {} {}", fileSpecification, nodeIdentifier, packName);

        var nodeInfo = _nodeGraph.get(nodeIdentifier);
        if (nodeInfo == null) {
            LOGGER.fatal(String.format("assignFixedDiskUnitToExec() Cannot find node %012o", nodeIdentifier));
            Exec.getInstance().stop(StopCode.FacilitiesComplex);
            throw new ExecStoppedException();
        }
        ...
        var node = nodeInfo.getNode();
        if ((node.getNodeCategory() != NodeCategory.Device) || ((Device) node).getDeviceType() != DeviceType.DiskDevice) {
            LOGGER.fatal(String.format("assignFixedDiskUnitToExec() Cannot assign to non-disk device %012o", nodeIdentifier));
            Exec.getInstance().stop(StopCode.FacilitiesComplex);
            throw new ExecStoppedException();
        }
    ...
    public synchronized boolean catalogDiskFile(
        ...
        LOGGER.traceEntry("catalogDiskFile {} {}", request, fsResult);
        ...
        try {
            ...
            LOGGER.fatal("file set {} should not already exist", fsInfo);
            exec.stop(StopCode.FacilitiesComplex);
            throw new ExecStoppedException();
        }
    ...
    public synchronized boolean catalogDiskFileCycle(
        ...
        LOGGER.traceEntry("catalogDiskFileCycle {} {}", run, request._fileSpecification.toString());
    ...
    public synchronized boolean catalogTapeFile(
        ...
        LOGGER.traceEntry("catalogTapeFile {} {}", fileSpecification, type);
    ...
    public synchronized boolean catalogTapeFileCycle(
        ...
        LOGGER.traceEntry("catalogTapeFileCycle {} {} {}", fileSpecification, fileSetInfo, type);
    ...
    public void establishUseItem(
        ...
        LOGGER.traceEntry("establishUseItem {} {} {}", run, internalName, fileSpecification, releaseOnTaskEnd);
        ...
        LOGGER.traceExit(run.getActualRunId());
    }
    ...
    public String getNodeStatusString(
        ...
        if (ni == null) {
            LOGGER.fatal("getNodeStatusString() Cannot find node {} in node graph", nodeIdentifier);
            Exec.getInstance().stop(StopCode.FacilitiesComplex);
            throw new ExecStoppedException();
        }
    ...
    public synchronized DiskFileFacilitiesItem ioGetDiskFileFacilitiesItem(
        ...
        switch (facItem) {
            case null -> {
                LOGGER.fatal("[{}] Cannot find facilities item for internal name {}", run.getActualRunId(), internalName);
                exec.stop(StopCode.InternalExecIOFailed);
                throw new ExecStoppedException();
            }
            case NameItem nameItem -> {
                LOGGER.fatal("[{}] File {} is not assigned", run.getActualRunId(), internalName);
                exec.stop(StopCode.InternalExecIOFailed);
                throw new ExecStoppedException();
            }
            ...
            default -> {
                LOGGER.fatal("[{}] File {} is not a disk file", run.getActualRunId(), facItem.getFilename());
                exec.stop(StopCode.InternalExecIOFailed);
                throw new ExecStoppedException();
            }
        }
    ...
    public synchronized void ioReadFromDiskFile(
        ...
        LOGGER.traceEntry("ioReadFromDiskFile {} {} {} {} {}", run, internalName, fileRelativeAddress, buffer, isWordAddressable);
        var exec = Exec.getInstance();
        var mm = exec.getMFDManager();
    ...
    public synchronized void ioWriteToDiskFile(
        ...
        LOGGER.traceEntry("ioWriteToDiskFile {} {} {} {} {}", run, internalName, fileRelativeAddress, buffer, isWordAddressable);
        var exec = Exec.getInstance();
        var mm = exec.getMFDManager();
    ...
    public boolean releaseFile(
        ...
        LOGGER.traceEntry("releaseFile {} {} {} {} {} {}", run, fileSpecification, behavior, deleteFileCycle, inhibitCatalog, releaseExclusiveUseOnly);
    ...
    public Channel routeIo(
        ...
        if (nodeInfo == null) {
            LOGGER.fatal("Node {} from channel program is not configured", nodeId);
            Exec.getInstance().stop(StopCode.FacilitiesComplex);
            throw new ExecStoppedException();
        }
        ...
        if (node.getNodeCategory() != NodeCategory.Device) {
            LOGGER.fatal("Node {} from channel program is not a device", nodeId);
            Exec.getInstance().stop(StopCode.FacilitiesComplex);
            throw new ExecStoppedException();
        }
    ...
    private void loadNodeGraph() throws KExecException {
        ...
        for (var configNode : nodes) {
            ...
            switch (configNode.getEquipType()) {
                ...
                case FILE_SYSTEM_PRINTER -> {
                    ...
                    if (path == null) {
                        LOGGER.fatal("Node {} has no configured PATH argument", nodeName);
                        error = true;
                        break;
                    }
        ...
        for (var cfgNode : nodes) {
            ...
            if (nodeInfo == null) {
                LOGGER.fatal("Node {} was not placed in the node graph", cfgName);
                error = true;
                continue;
            }
            ...
            if (!(node instanceof Channel channel)) {
                if (!cfgNode.getSubordinates().isEmpty()) {
                    LOGGER.fatal("Node {} is not a channel, but has subordinates", cfgName);
                    error = true;
                }
            ...
            for (var childCfgNode : cfgNode.getSubordinates()) {
                ...
                if (childNodeInfo == null) {
                    LOGGER.fatal("Node {} (child of {}) not found in node graph", childName, cfgName);
                    error = true;
                    continue;
                }
                ...
                if (!(childNode instanceof Device device)) {
                    LOGGER.fatal("Node {} (child of {}) is not a device", childName, cfgName);
                    error = true;
                    continue;
                }

                if (!channel.canAttach(device)) {
                    LOGGER.fatal("Node {} cannot attach to node {}", childName, cfgName);
                    error = true;
                    continue;
                }
            ...
            if ((ni.getNodeStatus() == NodeStatus.Up) || (ni.getNodeStatus() == NodeStatus.Suspended)) {
                if ((ni instanceof DeviceNodeInfo dni) && (ni.getNode() instanceof DiskDevice dd)) {
                    try {
                        ...
                        LOGGER.info("No route to device {}", dd.getNodeName());
                    }
                }
            }
        }
    ...
    private ArraySlice readInitialDirectoryTrack(
        ...
        if (ioPkt.getIoStatus() != IoStatus.Successful) {
            LOGGER.error("readPackLabel ioStatus={}", ioPkt.getIoStatus());
    ...
    Channel selectRoute(final Device device) throws ExecStoppedException, NoRouteForIOException {
        ...
        if (ni == null) {
            LOGGER.fatal("Cannot find NodeInfo for {}", device.getNodeName());
            Exec.getInstance().stop(StopCode.FacilitiesComplex);
            throw new ExecStoppedException();
        }
        ...
        if (ni instanceof DeviceNodeInfo dni) {
            for (int cx = 0; cx < dni._routes.size(); cx++) {
                ...
                if (chi == null) {
                    LOGGER.fatal("Cannot find NodeInfo for {}", chan.getNodeName());
                    Exec.getInstance().stop(StopCode.FacilitiesComplex);
                    throw new ExecStoppedException();
                }
            ...
            LOGGER.fatal("ni is not a DeviceNodeInfo for {}", device.getNodeName());
            Exec.getInstance().stop(StopCode.FacilitiesComplex);
            throw new ExecStoppedException();
        }
    }
