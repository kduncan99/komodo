...
import com.bearsnake.komodo.kexec.exec.genf.queues.*;
import com.bearsnake.komodo.kexec.facilities.*;
...
public class GenFileInterface {
    ...
    private static final Logger LOGGER = LogManager.getLogger(GenFileInterface.class);
    ...
    public synchronized void enqueue(
        ...
        LOGGER.trace("{} enqueueing {} on {}", run.getActualRunId(), fileSpecification, queue.getQueueName());
    ...
    public FacStatusResult assignInputFile(
        ...
        if (item == null) {
            LOGGER.error("Caller requested input file item at sector {} which does not exist", sectorAddress);
        ...
        if (!(item instanceof InputQueueItem iqi)) {
            LOGGER.error("Caller requested input file item at sector {} which is not an input item", sectorAddress);
            fsResult.mergeStatusBits(0_400010_000000L).postMessage(FacStatusCode.FileIsNotCataloged, null);
            return fsResult;
        }
    ...
    public void initialize() throws ExecStoppedException {
        ...
        if (!exec.catalogDiskFileForExec(QUALIFIER,
                                         ...
                                         9999)) {
            LOGGER.fatal("Cannot catalog GENF$");
            exec.stop(StopCode.FileAssignErrorOccurredDuringSystemInitialization);
            throw new ExecStoppedException();
        }

        var facResult = new FacStatusResult();
        if (!fm.assignCatalogedDiskFileToExec(FILE_SPECIFICATION, false, facResult) || (facResult.hasErrorMessages())) {
            LOGGER.fatal("Cannot assign GENF$");
            exec.stop(StopCode.FileAssignErrorOccurredDuringSystemInitialization);
            throw new ExecStoppedException();
        }
    ...
    public void recover() throws ExecStoppedException {
        ...
        if (!(item instanceof SystemItem si)) {
            LOGGER.fatal("GENF$ sector 0 is not a system item");
        ...
        for (addr = 1; addr < si.getSectorCount(); addr++) {
            ...
            if (item instanceof InputQueueItem iqi) {
                ...
                LOGGER.fatal("GENF$ sector {} is not an expected item type: {}", addr, item.getItemType().getCode());
                exec.stop(StopCode.UndefinedGENFType);
                throw new ExecStoppedException();
            }
        }
