...
/**
 * Add Negative Magnitude to Accumulator instruction
 * (ANMA) Adds the arithmetic inverse of teh absolute value of the content of U under j-field control
 * to the value in A(a), and stores it in A(a).
 */
public class ANMAFunction extends FixedFunction {

    public static final ANMAFunction INSTANCE = new ANMAFunction();

    private ANMAFunction() {
        super("ANMA");
        var fc = new FunctionCode(0_17);
    ...
    public boolean execute(
        ...
        var result = add36(engine, aReg.getW(), Math.abs(operand));
        aReg.setW(result);
