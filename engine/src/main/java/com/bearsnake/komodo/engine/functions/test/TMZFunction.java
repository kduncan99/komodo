 ...
 * (TMZ) Checks the value of U to see if it is minus (negative) zero.
 * If the test succeeds, skip the next instruction by incrementing the program counter.
 * Extended Mode only.
 */
public class TMZFunction extends Function {

    public static final TMZFunction INSTANCE = new TMZFunction();

    private TMZFunction() {
        super("TMZ");
        setExtendedModeFunctionCode(new FunctionCode(050).setAField(04));
