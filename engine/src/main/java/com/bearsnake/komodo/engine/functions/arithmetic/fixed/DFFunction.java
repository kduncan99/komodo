 ...
 * (DF) Divides the 72-bit signed integer dividend from A(a):A(a+1) shifted to the right by 1,
 * by the 36-bit signed divisor contained in U, storing the resulting quotient in A(a) and
 * the remainder in A(a+1).
 */
public class DFFunction extends FixedFunction {

    public static final DFFunction INSTANCE = new DFFunction();

    private DFFunction() {
        super("DF");
        var fc = new FunctionCode(0_36);
    ...
    public boolean execute(
        ...
        var biDividend = BigInteger.valueOf(dividendMSW).shiftLeft(36).or(BigInteger.valueOf(dividendLSW)).shiftRight(1);
