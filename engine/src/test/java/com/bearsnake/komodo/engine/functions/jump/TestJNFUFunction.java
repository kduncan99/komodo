...
public class TestJNFUFunction extends FunctionUnitTest {

    private long jnfuBM(long x, long h, long i, long u) {
        return fjaxhiu(0_74, 0_15, 0_01, x, h, i, u);
    }

    private long jnfuBM(long u) {
        return jnfuBM(0, 0, 0, u);
    }

    private long jnfuEM(long x, long u) {
        return fjaxu(0_74, 0_15, 0_01, x, u);
    }

    private long jnfuEM(long u) {
        return jnfuEM(0, u);
    }
    ...
    @Test
    public void testJNFU_BM() throws MachineInterrupt, EngineHaltedException {
        setupBM();
        var bank = _engine.getBaseRegister(12).getStorage();
        bank.set(0, jnfuBM(0_100));

        // Case 1: Characteristic Underflow set -> Should NOT jump
        _engine.getDesignatorRegister().setCharacteristicUnderflow(true);
        ...
        // Case 2: Characteristic Underflow clear -> Should jump
        _engine.getDesignatorRegister().setCharacteristicUnderflow(false);
        _engine.getProgramAddressRegister().setProgramCounter(0);
        _engine.cycle();
    ...
    @Test
    public void testJNFU_EM() throws MachineInterrupt, EngineHaltedException {
        setupEM();
        var bank = _engine.getBaseRegister(0).getStorage();
        bank.set(0, jnfuEM(0_100));
    ...
    @Test
    public void testJNFU_Indexed_BM() throws MachineInterrupt, EngineHaltedException {
        setupBM();
        _engine.getExecOrUserXRegister(3).setXM(0_10);
        var bank = _engine.getBaseRegister(12).getStorage();
        bank.set(0, jnfuBM(3, 0, 0, 0_100)); // jump to 0_100 + X3.m (0_10) = 0_110
    ...
    @Test
    public void testJNFU_Indirect_BM() throws MachineInterrupt, EngineHaltedException {
        setupBM();
        var bank = _engine.getBaseRegister(12).getStorage();
        bank.set(0, jnfuBM(0, 0, 1, 0_100)); // jump indirect via 0_100
    ...
    @Test
    public void testJNFU_Indexed_EM() throws MachineInterrupt, EngineHaltedException {
        setupEM();
        _engine.getExecOrUserXRegister(3).setXM(0_10);
        var bank = _engine.getBaseRegister(0).getStorage();
        bank.set(0, jnfuEM(3, 0_100)); // jump to 0_100 + X3.m (0_10) = 0_110
