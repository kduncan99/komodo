/*
 * Copyright (c) 2018-2026 by Kurt Duncan - All Rights Reserved
 */

package com.bearsnake.komodo.engine.functions.arithmetic.decimal;

import com.bearsnake.komodo.engine.*;
import com.bearsnake.komodo.engine.interrupts.MachineInterrupt;
import org.junit.jupiter.api.BeforeEach;
import org.junit.jupiter.api.Test;

import static org.junit.jupiter.api.Assertions.assertEquals;

/**
 * Add Decimal instruction
 * (ADE) Adds decimal (U) to decimal A(a) and stores the result in A(a).
 */
public class TestADEFunction extends TestDecimalFunction {

    private long ade(long a, long x, long u) {
        return fjaxu(0_07, 0_00, a, x, u);
    }

    private long ade(long a, long x, long h, long i, long u) {
        return fjaxhiu(0_07, 0_00, a, x, h, i, u);
    }

    @BeforeEach
    public void setup() {
        _engine = new Engine(this, this);
    }

    @Test
    public void testADE_Positive_EM() throws MachineInterrupt {
        var code = new long[0_1000];
        // 12,345,678 + 1,111,111 = 13,456,789
        code[0] = ade(4, 0, 0_400);
        code[1] = 0;
        code[0_400] = decWord(0, 1, 1, 1, 1, 1, 1, 1, POSITIVE_SIGN);
        _engine.getExecOrUserARegister(4).setW(decWord(1, 2, 3, 4, 5, 6, 7, 8, POSITIVE_SIGN));

        loadBaseRegister((short) 0, false, 0, 0_777, AbsoluteAddress.encodeToLong(0, 0), code);

        _engine.getDesignatorRegister().setBasicModeEnabled(false);
        _engine.getProgramAddressRegister().setProgramCounter(0).setBankDescriptorIndex(0);

        run();

        assertEquals(decWord(1, 3, 4, 5, 6, 7, 8, 9, POSITIVE_SIGN), _engine.getExecOrUserARegister(4).getW());
    }

    @Test
    public void testADE_Negative_EM() throws MachineInterrupt {
        var code = new long[0_1000];
        // 100 + (-50) = 50
        code[0] = ade(4, 0, 0_400);
        code[1] = 0;
        code[0_400] = decWord(0, 0, 0, 0, 0, 0, 5, 0, NEGATIVE_SIGN);
        _engine.getExecOrUserARegister(4).setW(decWord(0, 0, 0, 0, 0, 1, 0, 0, POSITIVE_SIGN));

        loadBaseRegister((short) 0, false, 0, 0_777, AbsoluteAddress.encodeToLong(0, 0), code);

        _engine.getDesignatorRegister().setBasicModeEnabled(false);
        _engine.getProgramAddressRegister().setProgramCounter(0).setBankDescriptorIndex(0);

        run();

        assertEquals(decWord(0, 0, 0, 0, 0, 0, 5, 0, POSITIVE_SIGN), _engine.getExecOrUserARegister(4).getW());
    }

    @Test
    public void testADE_Zero_EM() throws MachineInterrupt {
        var code = new long[0_1000];
        code[0] = ade(4, 0, 0_400);
        code[1] = 0;
        code[0_400] = decWord(0, 0, 0, 0, 0, 0, 0, 0, POSITIVE_SIGN);
        _engine.getExecOrUserARegister(4).setW(decWord(1, 2, 3, 4, 5, 6, 7, 8, POSITIVE_SIGN));

        loadBaseRegister((short) 0, false, 0, 0_777, AbsoluteAddress.encodeToLong(0, 0), code);

        _engine.getDesignatorRegister().setBasicModeEnabled(false);
        _engine.getProgramAddressRegister().setProgramCounter(0).setBankDescriptorIndex(0);

        run();

        assertEquals(decWord(1, 2, 3, 4, 5, 6, 7, 8, POSITIVE_SIGN), _engine.getExecOrUserARegister(4).getW());
    }

    @Test
    public void testADE_Positive_BM() throws MachineInterrupt {
        var code = new long[0_1000];
        // 10 + 20 = 30
        code[0] = ade(4, 0, 0_1400);
        code[1] = 0;
        code[0_400] = decWord(0, 0, 0, 0, 0, 0, 2, 0, POSITIVE_SIGN);
        _engine.getExecOrUserARegister(4).setW(decWord(0, 0, 0, 0, 0, 0, 1, 0, POSITIVE_SIGN));

        loadBaseRegister((short) 12, false, 0_1000, 0_1777, AbsoluteAddress.encodeToLong(0, 0), code);

        _engine.getDesignatorRegister().setBasicModeEnabled(true);
        _engine.getProgramAddressRegister().setProgramCounter(0_1000).setBankDescriptorIndex(0);

        run();

        assertEquals(decWord(0, 0, 0, 0, 0, 0, 3, 0, POSITIVE_SIGN), _engine.getExecOrUserARegister(4).getW());
    }

    @Test
    public void testADE_Indirect_BM() throws MachineInterrupt {
        var code = new long[0_1000];
        // ADE A4, *0_400
        code[0] = ade(4, 0, 0, 1, 0_1400);//fjaxu(0_07, 0_00, 4, 0, 0_200400); // I=1, U=400
        code[1] = 0;
        code[0_400] = 0_1600;
        code[0_600] = decWord(0, 0, 0, 0, 0, 0, 0, 5, POSITIVE_SIGN);
        _engine.getExecOrUserARegister(4).setW(decWord(0, 0, 0, 0, 0, 0, 0, 5, POSITIVE_SIGN));

        loadBaseRegister((short) 14, false, 0_1000, 0_1777, AbsoluteAddress.encodeToLong(0, 0), code);

        _engine.getDesignatorRegister()
                .setBasicModeEnabled(true)
                .setProcessorPrivilege((short)2); 
        _engine.getProgramAddressRegister().setProgramCounter(0_1000).setBankDescriptorIndex(0_4); // bank 14 is BDI 4

        run();

        assertEquals(decWord(0, 0, 0, 0, 0, 0, 1, 0, POSITIVE_SIGN), _engine.getExecOrUserARegister(4).getW());
    }

    @Test
    public void testADE_Indexed_EM() throws MachineInterrupt {
        var code = new long[0_1000];
        // ADE A4, 0_300, X1
        code[0] = ade(4, 1, 0_300);
        code[1] = 0;
        code[0_400] = decWord(0, 0, 0, 0, 1, 1, 1, 1, POSITIVE_SIGN);

        loadBaseRegister((short) 0, false, 0, 0_777, AbsoluteAddress.encodeToLong(0, 0), code);

        _engine.getExecOrUserARegister(4).setW(decWord(0, 0, 0, 0, 2, 2, 2, 2, POSITIVE_SIGN));
        _engine.getDesignatorRegister().setBasicModeEnabled(false);
        _engine.getProgramAddressRegister().setProgramCounter(0).setBankDescriptorIndex(0);
        _engine.getExecOrUserXRegister(1).setW(0_100);

        run();

        assertEquals(decWord(0, 0, 0, 0, 3, 3, 3, 3, POSITIVE_SIGN), _engine.getExecOrUserARegister(4).getW());
    }

    @Test
    public void testADE_GRS_EM() throws MachineInterrupt {
        var code = new long[0_1000];
        // ADE A4, 0_100 (U=R0)
        code[0] = ade(4, 0, 0_100);
        code[1] = 0;

        loadBaseRegister((short) 0, false, 0, 0_777, AbsoluteAddress.encodeToLong(0, 0), code);

        _engine.getExecOrUserARegister(4).setW(decWord(0, 0, 0, 0, 2, 2, 2, 2, POSITIVE_SIGN));
        _engine.getExecOrUserRRegister(0).setW(decWord(0, 0, 0, 0, 1, 1, 1, 1, POSITIVE_SIGN));

        _engine.getDesignatorRegister().setBasicModeEnabled(false);
        _engine.getProgramAddressRegister().setProgramCounter(0).setBankDescriptorIndex(0);

        run();

        assertEquals(decWord(0, 0, 0, 0, 3, 3, 3, 3, POSITIVE_SIGN), _engine.getExecOrUserARegister(4).getW());
    }
}
