...
public class TestJNDFFunction extends FunctionUnitTest {

    private long jndfBM(long x, long h, long i, long u) {
        return fjaxhiu(0_74, 0_15, 0_03, x, h, i, u);
    }

    private long jndfBM(long u) {
        return jndfBM(0, 0, 0, u);
    }

    private long jndfEM(long x, long u) {
        return fjaxu(0_74, 0_15, 0_03, x, u);
    }

    private long jndfEM(long u) {
        return jndfEM(0, u);
    }
    ...
    @Test
    public void testJNDF_BM() throws MachineInterrupt, EngineHaltedException {
        setupBM();
        var bank = _engine.getBaseRegister(12).getStorage();
        bank.set(0, jndfBM(0_100));

        // Case 1: Divide Check set -> Should NOT jump
        _engine.getDesignatorRegister().setDivideCheck(true);
        ...
        // Case 2: Divide Check clear -> Should jump
        _engine.getDesignatorRegister().setDivideCheck(false);
        _engine.getProgramAddressRegister().setProgramCounter(0);
        _engine.cycle();
    ...
    @Test
    public void testJNDF_EM() throws MachineInterrupt, EngineHaltedException {
        setupEM();
        var bank = _engine.getBaseRegister(0).getStorage();
        bank.set(0, jndfEM(0_100));
    ...
    @Test
    public void testJNDF_Indexed_BM() throws MachineInterrupt, EngineHaltedException {
        setupBM();
        _engine.getExecOrUserXRegister(3).setXM(0_10);
        var bank = _engine.getBaseRegister(12).getStorage();
        bank.set(0, jndfBM(3, 0, 0, 0_100)); // jump to 0_100 + X3.m (0_10) = 0_110
    ...
    @Test
    public void testJNDF_Indirect_BM() throws MachineInterrupt, EngineHaltedException {
        setupBM();
        var bank = _engine.getBaseRegister(12).getStorage();
        bank.set(0, jndfBM(0, 0, 1, 0_100)); // jump indirect via 0_100
    ...
    @Test
    public void testJNDF_Indexed_EM() throws MachineInterrupt, EngineHaltedException {
        setupEM();
        _engine.getExecOrUserXRegister(3).setXM(0_10);
        var bank = _engine.getBaseRegister(0).getStorage();
        bank.set(0, jndfEM(3, 0_100)); // jump to 0_100 + X3.m (0_10) = 0_110
