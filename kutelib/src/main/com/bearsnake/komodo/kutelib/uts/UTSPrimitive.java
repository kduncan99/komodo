...
import static com.bearsnake.komodo.kutelib.Constants.ASCII_DC1;
import static com.bearsnake.komodo.kutelib.Constants.ASCII_DC2;
import static com.bearsnake.komodo.kutelib.Constants.ASCII_DC4;
...
public class UTSPrimitive {
     ...
     * @throws InvalidEscapeSequenceException If an invalid escape sequence is encountered.
     * @throws BufferOverflowException If the buffer is overflowed.
     */
    private static UTSPrimitive deserializeEscapePrimitive(final UTSByteBuffer source)
        ...
        return switch (source.getNext()) {
            ...
            default -> throw new InvalidEscapeSequenceException();
        };
    }
     ...
     * @throws CoordinateException If a coordinate-related error occurs during deserialization.
     * @throws IncompleteEscapeSequenceException If an escape sequence is incomplete.
     * @throws IncompleteFCCSequenceException If an FCC sequence is incomplete.
     * @throws InvalidEscapeSequenceException If an invalid escape sequence is encountered.
     * @throws InvalidFCCSequenceException If an invalid FCC sequence is encountered.
     */
     ...
     * @throws BufferOverflowException if the buffer is overflowed
     * @throws CoordinateException if the coordinate (or the byte[s] is/are invalid)
     */
    protected static int deserializeCoordinate(final UTSByteBuffer source)
        throws BufferOverflowException, CoordinateException {
        ...
        if ((ch1 >= 0x20) && (ch1 < 0x70)) {
            ...
            if (ch2 < 0x70) {
                throw new CoordinateException("Invalid double-byte coordinate");
            }
            return (ch1 - 0x75) * 16 + (ch2 & 0x0F) + 81;
        }
        throw new CoordinateException("Invalid single-byte coordinate");
    }
     ...
     * @throws CoordinateException if an invalid coordinate is deteected during serialization
     */
