...
public class InstructionProcessor extends Processor implements Worker {
    ...
    private long getOperand(
        ...
        int relAddress = calculateRelativeAddressForGRSOrStorage();
        boolean basicMode = _designatorRegister.getBasicModeEnabled();
        int pPriv = _designatorRegister.getProcessorPrivilege();
        int baseRegisterIndex = 0;
        if (!basicMode) {
            baseRegisterIndex = (int) _currentInstruction.getB();
            if ((pPriv < 2) && (_currentInstruction.getI() != 0)) {
        ...
        if ((grsCheck)
            && (basicMode || (baseRegisterIndex == 0))
            && (relAddress < 0200)) {
            incrementIndexRegisterInF0();

            //  First, do accessibility checks
            if (!GeneralRegisterSet.isAccessAllowed(relAddress, pPriv, false)) {
                throw new ReferenceViolationInterrupt(ReferenceViolationInterrupt.ErrorType.ReadAccessViolation, true);
            }
        ...
        if (basicMode) {
            baseRegisterIndex = findBaseRegisterIndex(relAddress, false);
        }
        BaseRegister baseRegister = _baseRegisters[baseRegisterIndex];
        baseRegister.checkAccessLimits(relAddress,
                                       false,
                                       true,
                                       false,
                                       _indicatorKeyRegister.getAccessInfo());
     ...
     */
    private void handleInterrupt() throws MachineInterrupt {
    ...
    private void storeOperand(
        ...
        if ((grsCheck)
            ...
            //  First, do accessibility checks
            if (!GeneralRegisterSet.isAccessAllowed(relAddress, pPriv, true)) {
    ...
    public void stop(
        ...
        EntryMessage em = _logger.traceEntry("stop({}, {})", stopReason, detail);
