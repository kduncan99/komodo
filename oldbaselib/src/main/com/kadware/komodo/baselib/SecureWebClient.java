...
public class SecureWebClient {
    ...
    public ResultFromSend send(
        ...
        LOGGER.debug(String.format("Sending to URL:%s operation=%s", url, operation.toString()));
        ...
        for (RequestProperty property : _properties) {
            LOGGER.debug(String.format("  Property %s=%s", property._key, property._value));
        ...
        LOGGER.debug(String.format("Result=%d:%s", responseCode, responseMessage));
        System.out.println(String.format("Result=%d:%s", responseCode, responseMessage));   //  TODO remove
        byte[] response;
        if (responseCode >= 300) {
        ...
        LOGGER.debug(String.format("<--%s", new String(response)));
        System.out.println(String.format("<--%s", new String(response)));   //  TODO remove
